...
{
	if (str[list->i] == 'c')
		ft_eprintf_c(va_arg(params, int), list);
	else if (str[list->i] == 's')
		ft_eprintf_s(va_arg(params, char *), list);
	else if (str[list->i] == 'i' || str[list->i] == 'd')
		ft_eprintf_id(va_arg(params, int), list);
	else if (str[list->i] == 'u')
		ft_eprintf_u(va_arg(params, unsigned int), list);
	else if (str[list->i] == 'p')
		ft_eprintf_p(va_arg(params, unsigned long int), list);
	else if (str[list->i] == 'x')
		ft_eprintf_x(va_arg(params, unsigned int), list, str);
	else if (str[list->i] == 'X')
		ft_eprintf_x(va_arg(params, unsigned int), list, str);
	else if (str[list->i] == 'o')
		ft_eprintf_o(va_arg(params, unsigned int), list, str);
	return (list->ret);
}
...
void	ft_echeck_percent(const char *str, t_list_printf *list, va_list params)
{
	if (str[list->i] == '%')
	{
		...
		if ((str[list->i] == '%'))
		{
			...
			ft_eprintf_conversion(str, list, params);
	}
}
...
int	ft_eprintf(const char *str, ...)
{
	...
	while (str[list.i])
	{
		if (str[list.i] != '%')
		{
			ft_putchar_fd(str[list.i], 2);
			list.ret++;
			list.i++;
		}
		ft_echeck_percent(str, &list, params);
	}
	va_end(params);
	return (list.ret);
}
